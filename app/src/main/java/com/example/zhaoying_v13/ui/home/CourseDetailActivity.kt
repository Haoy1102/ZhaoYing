...
import com.example.zhaoying_v13.ui.home.detection.DetectionActivity
...
class CourseDetailActivity : AppCompatActivity() {
    ...
    private val _sigupStatus = MutableLiveData<Int>()
    val sigupStatus: LiveData<Int>
        get() = _sigupStatus
    ...
    private fun updateDatabase() {
        lifecycleScope.launch {
            ...
            if (curentUserInfo == null) {
                _sigupStatus.value=0
                Toast.makeText(this@CourseDetailActivity, "请先登录", Toast.LENGTH_SHORT).show()
                return@launch
            } else {
                userSignUpCourse(User2Course(curentUserInfo.userID, courseID))
                _sigupStatus.value=1
                Toast.makeText(this@CourseDetailActivity, "报名成功！", Toast.LENGTH_SHORT).show()
                return@launch
            }
        ...
        lifecycleScope.launch {
            ...
            if (courseInfo == null)
                ...
                binding.courseGradeTextView.text =
                    courseInfo.grade + "  ·  " + courseInfo.timeConsumeing + "分钟"
